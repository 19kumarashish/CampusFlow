  ...
  return (
    <div className="flex flex-col sm:flex-row flex-wrap items-center gap-4 bg-slate-950/40 p-4 border border-slate-900 rounded-xl backdrop-blur-xl">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
        {/* Status Dropdown */}
        <select
          className="h-9 w-full rounded-md border border-slate-800 bg-slate-900/40 px-3 py-1.5 text-xs text-slate-300 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:w-[150px] transition-colors"
          value={status}
        ...
        <Button
          variant="outline"
          size="sm"
          className="h-9 w-full sm:w-auto border-slate-850 bg-slate-950 text-slate-405 hover:bg-slate-900 hover:text-white"
          onClick={onReset}
        >
