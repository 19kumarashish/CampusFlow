...
import { Plus, Loader2, ChevronLeft, ChevronRight, BookOpen } from "lucide-react";
...
export default function CoursesPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [search, status, degree, department]);

  const queryParams: GetCoursesParams = {
    page,
    limit,
    search: search.trim() || undefined,
    status: status !== "ALL" ? (status as CourseStatus) : undefined,
    degree: degree !== "ALL" ? (degree as DegreeType) : undefined,
    department: department !== "ALL" ? department : undefined,
  };

  const { data, isLoading, isError, refetch } = useCoursesQuery(queryParams);
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-900 pb-5">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-white flex items-center gap-2">
            <BookOpen className="h-6 w-6 text-indigo-400" /> Academic Courses
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Browse through core university courses, degree types, semesters, and duration.
          </p>
        </div>
        {isAdmin && (
          <Button
      ...
      {isLoading && !data ? (
        <div className="flex flex-col items-center justify-center p-20">
          ...
          <p className="text-xs text-slate-400 mt-3 font-medium">Fetching academic course directory...</p>
        </div>
      ) : isError ? (
        <div className="text-center p-12 bg-slate-900/10 border border-slate-850 rounded-xl my-6">
          <p className="text-red-400 text-xs font-semibold">Failed to fetch courses directory.</p>
          <Button
            size="sm"
            variant="outline"
          ...
          {data?.pagination && data.pagination.totalPages > 1 && (
            <div className="flex justify-between items-center bg-slate-950/20 px-4 py-3 border border-slate-900 rounded-lg">
              <span className="text-[11px] text-slate-500 font-medium">
                ...
                {data.pagination.total} total courses)
              </span>
